...
const LAT = -19.057018;
const LNG = -41.013637;
const LS_KEY = "msg_lida_hash_v2";

// ===== Persistência robusta (localStorage + window.name fallback) =====
...
// ===== Decodifica base64 com UTF-8 corretamente =====
...
const icone = L.divIcon({
  className: "",
  html: `
    <div class="pin-personalizado">
      <img src="./pin.png" alt="Pin personalizado" onerror="this.style.display='none'">
      <div class="badge-novo">1</div>
    </div>
  `,
  iconSize: [60, 60],
  iconAnchor: [30, 60],
...
marcador.on("click", function() {
  const overlay = document.getElementById("overlay-codigo");
  const campo = document.getElementById("codigo-input");
...
// Busca o badge DENTRO do elemento do marcador (não globalmente)
function getBadge() {
  const el = marcador.getElement();
  if (!el) return null;
  return el.querySelector(".badge-novo");
}

function setBadgeVisible(visivel) {
  const badge = getBadge();
  if (!badge) return;
  if (visivel) badge.classList.remove("oculto");
  else badge.classList.add("oculto");
}

async function checkBadge() {
  try {
    const resposta = await fetch("https://api.github.com/repos/" + REPO + "/contents/mensagem.txt?ref=main&_=" + Date.now(), { cache: "no-store" });
    if (!resposta.ok) throw new Error("HTTP " + resposta.status);
    const data = await resposta.json();
    const msg = decodeBase64UTF8(data.content).trim();
    const hashAtual = hash(msg);
    const hashLida = storageGet(LS_KEY);
    if (msg && hashAtual !== hashLida) setBadgeVisible(true);
    else setBadgeVisible(false);
  } catch (erro) {
    console.error("Erro ao verificar mensagem:", erro);
  }
}
...
async function mostrarMsg() {
  try {
    ...
    storageSet(LS_KEY, hash(msg));
    setBadgeVisible(false);
    document.getElementById("overlay-codigo").style.display = "none";
    document.getElementById("texto-secreto").textContent = msg;
    document.getElementById("overlay-msg").style.display = "flex";
  } catch (erro) {
    console.error("Erro ao abrir mensagem:", erro);
    resetar();
  }
}

function verificarCodigo() {
  const valor = document.getElementById("codigo-input").value.trim().toLowerCase();
  if (valor === CODIGO) mostrarMsg();
...
async function enviarResposta() {
  ...
  if (!texto) return;
  try {
    const resposta = await fetch("https://ntfy.sh/" + NTFY_TOPICO, {
...
document.getElementById("codigo-input").addEventListener("keypress", function(e) { if (e.key === "Enter") verificarCodigo(); });
document.getElementById("btn-enviar-resposta").addEventListener("click", enviarResposta);
document.getElementById("btn-queimar").addEventListener("click", resetar);
document.getElementById("overlay-codigo").addEventListener("click", function(e) { if (e.target === e.currentTarget) resetar(); });
document.getElementById("overlay-msg").addEventListener("click", function(e) { if (e.target === e.currentTarget) resetar(); });

// Aguarda o marcador estar no DOM antes de verificar o badge
if (marcador.getElement()) {
  checkBadge();
} else {
  marcador.once("add", checkBadge);
}
setInterval(checkBadge, 60000);
</script>
</body>
</html>
HTMLEOF
